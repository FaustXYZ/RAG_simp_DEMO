...
tool_descs='\n'.join(['%s: %s,args: %s' % (tool.name, tool.description, json.dumps([{'name': name, 'description': info.get('description', ''), 'type': info.get('type','')} 
        for name, info in tool.args.items()], ensure_ascii=False)) for tool in tools])
...
def agent_execute(query, chat_history=[],**kwargs):
    ...
    while True:
        ...
        prompt = prompt_tpl.format(today=today, chat_history=history, tool_description=tool_descs, tool_names=tool_names,
                                   input=query, agent_scratchpad=agent_scratchpad)
        print('\033[32m---等待LLM返回... ...\n%s\n\033[0m' % prompt, flush=True)
        response = llm_model.execute(prompt, history,user_stop_words=['Observation:'])
...
while True: # True, final_answer, chat_history
    ...
    result = agent_execute_with_retry(query=query, chat_history=my_history,tools_config=tools_config,prompt=PROMPT_PLT,model=model )
